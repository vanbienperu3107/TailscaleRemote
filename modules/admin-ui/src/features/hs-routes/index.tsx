...
function RouteRow({ route, onEnable, onDelete, loading }: {
  route: HsRoute
  onEnable: () => void
  onDelete: () => void
  loading?: boolean
}) {
  const nodeName = route.node?.givenName || route.node?.name || route.node?.id || '—'
  return (
    <TableRow>
    ...
    <Main className='flex flex-1 flex-col gap-4 sm:gap-6'>
      <div>
        ...
        <p className='text-muted-foreground'>
          Subnet routes các node quảng bá trong tailnet (DERP-Controller API). Tự làm mới 30s.
        </p>
      </div>
