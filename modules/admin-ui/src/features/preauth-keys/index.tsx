...
function CreateKeyDialog({
  ...
  const mut = useMutation({
    ...
    onSuccess: (key) => {
      const keyStr = typeof key === 'string' ? key : (key as HsPreAuthKey).key ?? ''
      setNewKey(keyStr)
...
function KeyRow({
  ...
  const expired =
    k.expiration ? new Date(k.expiration) < new Date() : false
  return (
    <TableRow>
...
export function PreAuthKeys() {
  ...
  const users = (usersQ.data?.users ?? []).map((u) => u.name ?? '').filter(Boolean)
  ...
  return (
    <Main className='flex flex-1 flex-col gap-4 sm:gap-6'>
      <div className='flex items-start justify-between'>
        ...
        <Button size='sm' onClick={() => setCreateOpen(true)} disabled={users.length === 0}>
                    ...
                    {(keysQ.data?.preAuthKeys ?? []).length === 0 ? (
                      ...
                      (keysQ.data?.preAuthKeys ?? []).map((k, i) => (
                        <KeyRow
                          ...
                          onExpire={() =>
                            k.user && k.key && expireMut.mutate({ user: k.user, key: k.key })
                          }
                        />
                      ))
                    )}
