...
export type HsRoute = {
  id?: string
  node?: { id?: string; name?: string; givenName?: string }
  prefix?: string
  advertised?: boolean
  enabled?: boolean
  isPrimary?: boolean
...
export async function expireNode(id: string): Promise<void> {
  await api.post(`/nodes/${id}/expire`)
}

export async function renameNode(id: string, name: string): Promise<void> {
  await api.post(`/nodes/${id}/rename`, { name })
}

export async function setNodeTags(id: string, tags: string[]): Promise<void> {
  await api.put(`/nodes/${id}/tags`, { tags })
}

// ─── Routes ──────────────────────────────────────────────────────────────────

export async function fetchRoutes(): Promise<{ configured: boolean; routes: HsRoute[] }> {
  const { data } = await api.get('/hs-routes')
  return data
}

export async function enableRoute(id: string): Promise<void> {
  await api.post(`/hs-routes/${id}/enable`)
}

export async function deleteRoute(id: string): Promise<void> {
  await api.delete(`/hs-routes/${id}`)
}

// ─── ACL ─────────────────────────────────────────────────────────────────────

export async function fetchAcl(): Promise<{ configured: boolean; policy: string }> {
  const { data } = await api.get('/acl')
  return data
}

export async function updateAcl(policy: string): Promise<void> {
  await api.put('/acl', { policy })
}
...
export async function fetchPreAuthKeys(
  user: string,
): Promise<{ configured: boolean; preAuthKeys: HsPreAuthKey[] }> {
  const { data } = await api.get('/preauthkeys', { params: { user } })
  return data
}
...
export async function createPreAuthKey(opts: {
  ...
  return data.preAuthKey ?? data
}

export async function expirePreAuthKey(user: string, key: string): Promise<void> {
  await api.post('/preauthkeys/expire', { user, key })
}
...
export async function renameUser(name: string, newName: string): Promise<void> {
  await api.post(`/users/${encodeURIComponent(name)}/rename`, { newName })
}
